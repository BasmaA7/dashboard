...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    //  $name = mysqli_real_escape_string($conx, $_POST['FullName']);
    // $password = mysqli_real_escape_string($conx, $_POST['Password']);
    // $hashedPassword = mysqli_real_escape_string($conx, $hashedPassword);
    // $email = mysqli_real_escape_string($conx, $_POST['Email']);
    // $role = mysqli_real_escape_string($conx, $_POST['role']);

    $sql = "INSERT INTO `user` (FullName, Password, Email, role) VALUES (?, ?, ?, ?)";
    $stmt = mysqli_prepare($conx, $sql);

    if ($stmt) {
        mysqli_stmt_bind_param($stmt, "ssss", $name, $hashedPassword, $email, $role);
...
<?php include "../nav_b.php";?>
 ...
 <!--  Form signUp -->
      ...
      <form  class="space-y-8 " method= 'post'>
        <div>
          <input type="text" id="nom" name="FullName"
        ...
        <div>
          <input type="email" id="subject" name="Email"
            class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-3xl focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500 dark:shadow-sm-light"
            placeholder="Email" >
        ...
        <div>
          <input type="password" id="password" name="Password"
            class="block p-3 w-full text-sm text-gray-900 bg-gray-50 rounded-3xl border border-gray-300 shadow-sm focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500 dark:shadow-sm-light"
            placeholder="password">
            <span class="text-red-600 hidden" id="password-error">no valid</span>
        </div>
        <!-- <div class="sm:col-span-2">
          <input id="passwordConfirm" rows="6"
            class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-3xl shadow-sm border border-gray-300 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
            placeholder="Confirm password">
            <span class="text-red-600 hidden" id="confirm-error">no valid</span>
        </div> -->
        <div>
   <select name="role" class="block p-3 w-full text-sm text-gray-900 bg-gray-50 rounded-3xl border border-gray-300 shadow-sm focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500 dark:shadow-sm-light" >
        ...
        <div class="flex justify-center">
          <div> 
            <button type="submit" name="submit"
              class="py-3 text-sm font-medium text-center text-white rounded-3xl bg-orange-600 w-60 hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800 ">Start</button>
          </div>
        </div>
      </form>

      <div class=" flex justify-center">
      <div> <h2 class="text-orange-500 font-semibold pt-4 mt-4 ">Forgot Password ?</h2></div>
      </div> 
      <div class="flex justify-center dark:text-white mt-4 ">  <p>Don't have an account? <span class="">SignUp</span></p></div> 
    </div>
  </section> 
